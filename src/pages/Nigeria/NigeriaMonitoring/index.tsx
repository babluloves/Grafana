...
const NigeriaMonitoring: React.FC = () => {
  ...
  const [data5, setData5] = useState<Record<string, Record<number, number>>>({});
  ...
  useEffect(() => {
    ...
    const CISDataForDisplay = (response: any): Record<string, Record<number, number>> => {
      const organizedData: Record<string, Record<number, number>> = {};

      response.results.forEach((result: any) => {
        result.series.forEach((series: any) => {
          series.values.forEach(([timestamp, value]: [number, number]) => {
            const timestampWithoutNanos = Math.floor(timestamp / 1000000);
            const date = convertUnixTimeToHumanReadable(timestampWithoutNanos).split(',')[0];
    ...
    const fetchData = async () => {
      setLoading(true);
      try {
        const url = `${backendUrl}/proxy/nigeria/activations`;
  ...
  return (
    <div className="App">
      {loading ? (
        ...
        <>
          <h2>Nigeria Monitoring Sheet</h2>
          ...
          <Table title="CIS Count's" data={data5} />
        </>
      )}
      <ToastContainer />
    </div>
